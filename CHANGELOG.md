...
- **Performance-only desktop** — the Visual mode (Desktop Cube, Coverflow Alt-Tab, Blur My Shell, per-workspace `x47-ws-walls`) and the PERF/VISUAL top-bar switcher are retired and uninstalled on sight. With the window animations already gone, the switch only cost RAM and shell stability. `--desktop-mode` is gone; `x47-desktop-mode`, `x47-power-desktop-sync`, and the adaptive-display autostart are removed. Dock stays hidden (Super / lime Show Apps is the launcher).
- **X47 Updates checks everything** — the report now covers the Ubuntu release channel (LTS + interim + in-development, e.g. 26.10), firmware (fwupd), pipx, cargo crates, go tools, ruby gems, GitHub release binaries, and ~/tools git clones. Apply upgrades all of them, not just apt/snap/Cursor/VS Code.
- **App grid fully foldered** — every visible app now lives in a folder: X47 (Settings, Updates, Sync, Ark), Internet, Office, Media, Utilities, System, Dev Tools, Pentesting. Duplicates removed (PDF tools were in both Utilities and Office); NymVPN (replaced by Mullvad), TeX doc viewers, lstopo, and the duplicate firmware-updater entry are hidden via `NoDisplay` overrides. Layout is restored from `assets/manifests/app-folders.dconf` on install.
...
- **Ark site page** — [ark.html](https://sk1tzwzd.github.io/ubuntu-x47-build/ark.html) plus an Ark nav item.
...
### Desktop modes
- **Dock off in both modes** — Visual no longer re-shows the bottom dock; hide via `manualhide` (Ubuntu session owns `ubuntu-dock`).
- **Nerovia Firefox widgets retired** — autostart/helper stubbed; no longer spawn on login or mode switch.
...
### Cursor
- **Autohide title bar** when unmaximized (Cursor) — later **removed**; soft-hide kept Firefox-only.
...
### Top bar
- **Show Apps** circle stays at **40px**; reverted the oversized 50px / 58px panel scale (default bar + status icons again).
...
### Launcher
- **Ubuntu Dock removed** — use Super (Activities / app grid) and Super+1…9 for favorites on main and anon.
...
- **Kept:** CTRL-only tiling, notification click-to-focus, X47 wallpapers / Show Apps icon.
